...
CREATE OR REPLACE FUNCTION try_reserve_new_order_message(interval_seconds INT)
...
BEGIN
    FOR rec IN
        WITH grouped AS (
            ...
            FROM ""new-order-messages""
            GROUP BY user_id
        ),
        earliest_per_user AS (
            SELECT o.order_id
            FROM ""new-order-messages"" o
...
$$;");
        ...
        {
            migrationBuilder.Sql(@"DROP FUNCTION IF EXISTS try_reserve_new_order_message;");
