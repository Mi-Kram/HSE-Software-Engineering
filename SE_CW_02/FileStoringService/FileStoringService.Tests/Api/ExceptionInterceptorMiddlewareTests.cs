...
namespace FileStoringService.Tests.Api
{
    public class ExceptionInterceptorMiddlewareTests
    {
        ...
        [Fact]
        public async Task Handle_ShouldReturn208_WhenWorkAlreadyUploadedExceptionThrown()
        {
            var loggerMock = new Mock<ILogger<ExceptionInterceptorMiddleware>>();
            var middleware = new ExceptionInterceptorMiddleware(loggerMock.Object);
            ...
            Task Next() => throw new WorkAlreadyUploadedException(42);
            ...
            Assert.Equal(StatusCodes.Status208AlreadyReported, context.Response.StatusCode);
            Assert.Equal(42, json.RootElement.GetProperty("workID").GetInt32());
        }
